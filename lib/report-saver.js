            ...
            return `
                ...
                <div class="card">
                    <div class="card-header" id="heading${id}">
                        <a class="mb-0 d-flex justify-content-between" data-toggle="collapse" data-target="#collapse${id}" aria-expanded="true" aria-controls="collapse${id}">
                            <span>
                                ...
                                ${curr.issue === '404' ? 'http 404' : `${curr.diff.metadata.add} add / ${curr.diff.metadata.del} del`}
                            </span>
                    ...
                    <div id="collapse${id}" class="collapse show" aria-labelledby="heading${id}" data-parent="#accordionExample">
                        <div class="card-body">
                        ${curr.issue === 'diff' ? diffAsEllipsedHtml(curr.diff.diff, snippetContextSize) : `<a href="${curr.fetchedUrl}" target="_blank">${curr.fetchedUrl}</a> returned a 404`}
                        </div>
                    </div>
                </div>`
            }, '')}`)
