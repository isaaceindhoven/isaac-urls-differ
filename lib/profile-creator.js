...
const exitIfProfileFolderAlreadyExists = async (profileConfigFile, profileName) => {
    if (await util.doesDirExist(profileConfigFile)) {
        util.logError(`Oops. Folder for profile '${profileName}' already exists.`)
        process.exit(1)
    }
}
...
module.exports = async (profileName) => {
    ...
    console.log(`Succes! Profile '${profileName}' folder and default config created in '${profileFolder}'.`)
}